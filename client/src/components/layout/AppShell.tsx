...
const routePathByView: Record<AppRoute, string> = {
  canvas: '/canvas',
  gallery: '/gallery',
};

const readCurrentRoute = (): AppRoute => (window.location.pathname.startsWith('/canvas') ? 'canvas' : 'gallery');
...
export function AppShell() {
  ...
  const [appRoute, setAppRoute] = useState<AppRoute>(() => readCurrentRoute());
  ...
  const [pendingLoadedWork, setPendingLoadedWork] = useState<SavedCanvasWork | null>(null);

  const navigateToRoute = useCallback((nextRoute: AppRoute) => {
    const nextPath = routePathByView[nextRoute];
  ...
  useEffect(() => {
    if (window.location.pathname === '/') {
      window.history.replaceState(null, '', routePathByView.gallery);
    }

    const handlePopState = () => {
      setAppRoute(readCurrentRoute());
    };
  ...
  useEffect(() => {
    if (appRoute !== 'canvas' || !pendingLoadedWork || !canvasStageRef.current) {
      return;
    }

    canvasStageRef.current.loadSavedWork(pendingLoadedWork);
    setPendingLoadedWork(null);
  }, [appRoute, pendingLoadedWork]);
  ...
  const handleSaveWork = () => {
    ...
    void storageState.saveCurrentWork(
        {
          canvasSize: {
            height: 960,
            width: 1440,
        ...
        (savedWork) => setActiveWorkId(savedWork.id),
        (error: unknown) => {
  ...
  const handleLoadWork = (work: SavedCanvasWork) => {
    void storageState.loadWork(work).then((loadedWork) => canvasStageRef.current?.loadSavedWork(loadedWork)).then(
      () => {
  ...
  const handleDeleteWork = (workId: string) => {
    ...
    void storageState.deleteWork(workId).then(
      () => {
      ...
      (error: unknown) => {
        storageState.setErrorMessage(error instanceof Error ? error.message : '저장된 작업을 삭제하지 못했습니다.');
      },
    );
  };

  const handleOpenGalleryWork = (work: SavedCanvasWork) => {
    void storageState.loadWork(work).then(
      (loadedWork) => {
        setPendingLoadedWork(loadedWork);
        setActiveWorkId(work.id);
  ...
  return appRoute === 'gallery' ? (
    <div className="app-shell gallery-shell">
      <GalleryView
        ...
        onClearError={() => storageState.setErrorMessage('')}
        onDeleteWork={handleDeleteWork}
        onLoadWork={handleOpenGalleryWork}
        onModeChange={storageState.setMode}
        onNewCanvas={handleStartNewCanvas}
        onRefresh={storageState.refreshSavedWorks}
        savedWorks={storageState.savedWorks}
      />
    </div>
  ) : (
